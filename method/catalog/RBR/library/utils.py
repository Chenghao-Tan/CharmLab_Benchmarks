...
Light restructuring and modifications have been made in order to make it compatible with our structure.
...
# ---------- high-level RBR generator (wrapper) ----------
...
def rbr_recourse(
    ...
    best_x_b = None
    best_dist = torch.tensor(float("inf"), device=device)
    ...
    if (y_feas == 1).any():
        ...
        X_feas_pos = torch.empty((0, X_feas.shape[1]), device=device)


    if (y_feas == 0).any():
        X_feas_neg = X_feas[y_feas == 0].reshape([int((y_feas == 0).sum().item()), -1])
    else:
        X_feas_neg = torch.empty((0, X_feas.shape[1]), device=device)
    ...
    for t in range(max_iter):
        ...
        # again not used in the original code.
        # if cat_features_indices:
