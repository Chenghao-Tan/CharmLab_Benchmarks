...
def graph_search(
    ...
    immutable_constraint_matrix1 = np.ones((n, n)) # incase there are no immutable features, we want to have a matrix of ones
    immutable_constraint_matrix2 = np.ones((n, n))
...
def build_graph(
    ...
    """
    if is_knn:
        max_neighbors = data.values.shape[0] - 1
        n = min(n, max_neighbors)  # clamp before calling kneighbors_graph
